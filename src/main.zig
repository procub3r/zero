...
pub fn main() !void {
    ...
    var site_dir = std.fs.cwd().openDir(SITE_DIR, .{}) catch |err| {
        std.log.err("couldn't open site directory \"" ++ SITE_DIR ++ "\"", .{});
        return err;
    };
    defer site_dir.close();
    // Open source directory to iterate through all markdown files and render them
    var source_dir = std.fs.cwd().openDir(SOURCE_DIR, .{ .iterate = true }) catch |err| {
        std.log.err("couldn't open source directory \"" ++ SOURCE_DIR ++ "\"", .{});
        return err;
    };
    ...
    while (try source_walker.next()) |f| {
        ...
        const post = try Post.initAndRender(gpa, html_writer.writer(), source);
